...
| `broker_identity_features.py` | **ML V2** | Fitur **identitas broker** — net flow trailing 5d/10d untuk 19 broker yang muncul di ≥70% ticker-day, dinormalisasi terhadap turnover jendela yang sama, plus observable inventory (60d, lot dipulihkan eksak dari `netval`) dengan konsentrasi HHI/top1/top3 dan velocity. Menjawab pertanyaan yang enam statistik agregat lama **tidak bisa** ajukan: bukan *berapa* flow-nya, tapi **siapa**. |
| `model_v2_ablation.py` | **ML V2** | Experiment #1 roadmap: A price-only / B broker-agregat / C identity / D identity+inventory, di **baris identik**, dengan **purge/embargo** sesuai horizon target, dinilai pakai **IC cross-sectional harian + ICIR** (bukan pooled) dan excess return. Hasil 2026-08-30: **identity menang 1 dari 6** — lihat `HANDOFF.md` Lampiran S. Gate §43 berarti layer owner-affinity **belum boleh** dimulai. |
