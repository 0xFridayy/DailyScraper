...
def build_panel_with_multiday(conn):
    ...
    for days in (5, 10):
        invalid = px.set_index(["ticker", "date"])[f"lag_{days}"].isna()
        keys = pd.MultiIndex.from_frame(panel[["ticker", "date"]])
        mask = invalid.reindex(keys, fill_value=True).to_numpy()
        cols = [c for c in MULTI_DAY_BROKER if c.endswith(str(days))]
